      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title text-lg font-semibold" id="exampleModalLabel" v-if="editMode">Update Roles</h5>
            <h5 class="modal-title text-lg font-semibold" id="exampleModalLabel" v-else>Create Roles</h5>
          ...
          <div class="modal-body">
            <form @submit.prevent="save">
              <div class="form-group" :class="errors.name ? 'has-error' :''">
                ...
                <input
                  type="text"
                  class="form-control text-xs rounded-md"
              ...
              <div class="form-group" :class="errors.permission ? 'has-error' :''">
                <Multiselect
                  ...
                  :classes="{
                    ...
                    tags: 'flex-grow flex-shrink flex flex-wrap items-center mt-1 pl-2 rtl:pl-0 rtl:pr-2 overflow-auto list_roles',
                    tag: 'bg-red-600 text-white text-sm font-semibold py-0.5 pl-2 rounded mr-1 mb-1 flex items-center whitespace-nowrap rtl:pl-0 rtl:pr-2 rtl:mr-0 rtl:ml-1',}"
                />
              </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
              <thead class="bg-gray-50">
                <tr>
                  <th
                    scope="col"
                    class="px-6 py-3 text-left text-sm font-back text-gray-500 uppercase tracking-wider"
                  >id</th>
                  <th
                    scope="col"
                    class="px-6 py-3 text-left text-sm font-back text-gray-500 uppercase tracking-wider"
                ...
                <tr v-for="role in  roles.data" :key="role.id">
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="flex items-center">
                      <div class="ml-4">
                        <div class="text-base  font-medium text-gray-900">{{ role.id }}</div>
                      </div>
                    </div>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="text-base text-gray-900">{{ role.name }}</div>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
                    <span
                      v-for="permission in role.permissions"
                      :key="permission.id"
                      class="bg-gray-600 text-gray-100 text-base px-2 mx-1 py-1 rounded"
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <button
                      data-toggle="modal"
                      data-target="#exampleModal"
                      class="inline-block px-3 py-2 bg-gray-200 text-gray-700 font-black text-base leading-tight uppercase rounded shadow-md hover:bg-gray-300 hover:shadow-lg focus:bg-gray-300 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-gray-400 active:shadow-lg transition duration-150 ease-in-out"
                      @click="edit(role)"
                    >Update</button>
                    <button
                      @click="deleteRow(role.id)"
                      class="inline-block px-3 py-2 bg-gray-800 text-white font-black text-base leading-tight uppercase rounded shadow-md hover:bg-gray-900 hover:shadow-lg focus:bg-gray-900 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-gray-900 active:shadow-lg transition duration-150 ease-in-out"
                    >Delete</button>
                  </td>
                </tr>
              </tbody>
...
::-webkit-scrollbar-thumb:hover {
  background: rgba(85, 85, 85, 0.452); 
}
</style>