<template>
  <section class="content">
    ...
    <button
      type="button"
      class="inline-block px-3 py-2 bg-blue-600 text-white font-black text-sm leading-tight uppercase rounded shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out"
      data-toggle="modal"
              ...
              <div class="relative p-6 flex-auto">
                <div class="flex flex-wrap -mx-3 mb-6">
                  <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                    ...
                    >{{__('name')}}</label>
                    <input
                      class="appearance-none block w-full bg-gray-200 text-gray-500 border rounded py-2 px-3 mb-3 text-sm leading-tight focus:outline-none focus:bg-white"
                      id="grid-first-name"
                      type="text"
                      placeholder
                      v-model="form.name"
                    ...
                    />
                    <p class="text-red-500 text-sm italic" v-if="errors.name">{{ errors.name }}</p>
                  </div>
                  <div class="w-full md:w-1/2 px-3">
                    ...
                    >{{__('email')}}</label>
                    <input
                      class="appearance-none block w-full bg-gray-200 text-gray-700 border text-sm border-gray-200 rounded py-4 px-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                      id="grid-last-name"
                      type="text"
                    ...
                    />
                    <p class="text-red-500 text-sm italic" v-if="errors.email">{{ errors.email }}</p>
                  </div>
                </div>
                <div class="flex flex-wrap -mx-3 mb-6">
                  <div class="w-full md:w-1/2 px-3">
                    ...
                    />
                    <p class="text-red-500 text-sm italic" v-if="errors.phone">{{ errors.phone }}</p>
                  </div>
                  ...
                  <div class="w-full md:w-1/2 px-3">
                    ...
                    >{{__('password')}}</label>
                    <input
                      class="appearance-none block w-full bg-gray-200 text-gray-500 border rounded py-4 px-3 mb-3 text-sm leading-tight focus:outline-none focus:bg-white"
                      id="grid-first-name"
                      type="text"
                      placeholder
    ...
    <div class="w-full max-w-md mr-4 mb-8 mt-8">
      <input
        v-model="term"
        @keyup="search"
        class="relative w-full px-8 py-3 text-sm rounded-r focus:shadow-outline"
        autocomplete="off"
        type="text"
        name="search"
        placeholder="Search…"
            ...
            <table class="min-w-full divide-y divide-gray-200">
              <thead class="bg-gray-50">
                <tr>
                  <th
                    scope="col"
                    class="px-2 py-2 text-left text-sm font-back text-gray-500 uppercase tracking-wider"
                  >id</th>
                  <th
                    scope="col"
                ...
                <tr v-for="(user,index) in  users.data" :key="index">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap">
                    <span
                      ...
                      :key="role.id"
                      class="bg-gray-600 text-gray-100 text-sm px-2 mx-1 py-1 rounded"
                    >{{ role.name }}</span>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div
